...
# (`!/General Human Knowledge/`) must make a file dropped under General Human Knowledge/
...
# record IS. Revert-proof: remove the `!/General Human Knowledge/` line from .gitignore and the
...
case_record_whitelisted() {
  ...
  R85_STAGED=$(git add -A --dry-run 2>/dev/null \
    | grep -F 'General Human Knowledge/whitelist-probe' || true)
  [ -n "$R85_STAGED" ] \
    || { echo "BUG [record-whitelisted]: a file under General Human Knowledge/ would NOT be" \
           "staged — the whitelist re-include is missing or broken."; rm -f "$R85_PROBE"; \
         exit 1; }
  rm -f "$R85_PROBE"
  echo "  ok [record-whitelisted] — General Human Knowledge/ is inside the whitelist (a probe" \
