...
# rewritten, so the report reads exactly as it did. Two conventions keep that move faithful:
#   - the checks share GLOBAL variables (the moved code declares no `local`), which is how the old
#     top-level sequence passed values along — $fails above all;
...
#     that function's RETURN value — which `set -e` would treat as a reason to abort the report
#     mid-way. The explicit return keeps the old meaning; a failure that aborted before still
#     aborts, since `set -e` is just as live inside a function as it was at the top level.
set -euo pipefail
...
# record when a condition began, and status is the only observer present at onset, so this
# first-seen day is a PRIMARY OBSERVATION, not a derived view — status still stores nothing
# derived. Ruling 4a puts the record inside the estate whitelist (the aging record is itself part
...
# fortnight, a yellow that outlived one sprint; 45 = ~six weeks, past the monthly review it was
# meant to prompt; 90 = a quarter, a yellow nobody can still call recent. Yellow stays yellow at
# EVERY tier — only the typographic WEIGHT escalates, never the severity or the exit code.
...
WARN_STATE_NOTE=""    # a one-line NOTE if the record degraded this run; printed near the verdict
WARN_ACTIVE=""        # "<epoch>\t<key>" for every WARN seen THIS run (drives the reconcile)

# load_warn_state — read the state ONCE, fails-open (A3/A4): an unreadable OR unparseable/corrupt
# file is treated as ABSENT — status regenerates from the current WARN set and prints one note, it
# never dies. Guarded so no read failure can trip set -e mid-report.
load_warn_state() {
  [[ -f "$HARNESS_WARN_STATE_FILE" ]] || return 0
  if ! WARN_STATE_OLD=$(cat "$HARNESS_WARN_STATE_FILE" 2>/dev/null); then
    WARN_STATE_OLD=""
    WARN_STATE_NOTE="NOTE: WARN-aging state unreadable at $HARNESS_WARN_STATE_FILE —"
    WARN_STATE_NOTE="$WARN_STATE_NOTE ages reset this run."
    return 0
  fi
  ...
  if [[ -n "$WARN_STATE_OLD" ]] \
    ...
    WARN_STATE_OLD=""
    WARN_STATE_NOTE="NOTE: WARN-aging state was corrupt at $HARNESS_WARN_STATE_FILE —"
    WARN_STATE_NOTE="$WARN_STATE_NOTE ages reset this run (regenerated from the current WARN set)."
  fi
  return 0
}
...
warn_age_suffix() {
  local d="$1"
  if (( d >= HARNESS_WARN_AGE_ALARM_DAYS )); then
    printf ' [!!! parked %sd — past the %sd mark]' "$d" "$HARNESS_WARN_AGE_ALARM_DAYS"
...
# warn_state_sync — reconcile + atomically persist the first-seen record ONCE, at the end, just
# before the verdict. NO-CHURN (A5): write ONLY when the active WARN set differs from what's on disk
# — first-seen kept for a persisting WARN, added when one appears, PRUNED when one clears (so a WARN
# that clears and returns starts a NEW episode at age zero — "how long has THIS sat" is the current
# continuous episode). Because the estate auto-commits every write, a touch-every-run file would
# mint a commit per status run and eat the G3 bloat budget; writing only on a real change avoids
# that. ATOMIC (A4): temp-file-then-rename kills torn files at the source; no locks (single-user
# estate). FAILS-OPEN (A3): a write to an unwritable/missing path must NOT abort the (already-
# printed) report or change the verdict rc — on any failure we print ONE note and return. Same law
# #79 shipped for its recorder: bookkeeping failure never changes the tool's answer.
warn_state_sync() {
  ...
  { mkdir -p "$dir" 2>/dev/null \
      ...
      && mv "$tmp" "$HARNESS_WARN_STATE_FILE" 2>/dev/null; } && return 0
  [[ -n "${tmp:-}" ]] && rm -f "$tmp" 2>/dev/null
  echo "NOTE: aging unavailable: state file unwritable at $HARNESS_WARN_STATE_FILE" \
       "(report is complete; ages reset next run)."
  return 0
}

# ticket_names — the estate's CONFORMING ticket folder names, one per line, matched against the
# SHARED $TICKET_RE. The commit-lag check and the per-ticket roster both read this same set; it was
# the same expression written out twice, so it has one home now.
ticket_names() {
  for d in "$WORK_ROOT/Tickets"/*/; do [[ -d "$d" ]] && basename "$d"; done 2>/dev/null \
  ...
  for f in check_ticket_log.sh harness-status.sh ticket-grammar.sh portability.sh \
    ...
    [[ -f "$p" ]] || {
      echo "FAIL: missing script $f. Fix: restore from git:" \
           "git -C '$WORK_ROOT' checkout -- '_harness/scripts/$f'"
...
# check_git_liveness — is there a record repo, and is it local-only?
check_git_liveness() {
  if ! git -C "$WORK_ROOT" rev-parse --git-dir >/dev/null 2>&1; then
    echo "FAIL: no git repo at $WORK_ROOT." \
         "Fix: git -C '$WORK_ROOT' init (whitelist .gitignore already present)."
    fails=$((fails+1))
    return 0
  fi
  last=$(git -C "$WORK_ROOT" log -1 --format=%cr 2>/dev/null || echo "never")
  echo "OK: work repo present; last commit $last."
  git -C "$WORK_ROOT" remote | grep -q . || return 0
  if [[ -n "${HARNESS_DEMO:-}" ]]; then
    echo "NOTE: remote present — fine for a template clone; your real Work repo must have none."
    return 0
  fi
  echo "FAIL: work repo has a REMOTE configured — it must be local-only." \
       "Fix: git -C '$WORK_ROOT' remote remove <name>"
  fails=$((fails+1))
  return 0
}

# check_commit_lag — commit-vs-session liveness cross-check (issue #4 / R-11): the auto-commit hook
# should capture every write, so the last commit must never lag behind session activity. If the
# newest session log entry across all tickets is meaningfully NEWER than the last commit, the safety
# net may have silently stopped firing — the exact failure the hook exists to prevent. Surface it
# (WARN, exit stays 0 — a yellow nudge, not a block). It compares WHEN work happened (session
# headers, local time) against WHEN it was last committed (git commit time). A margin (default 300s,
# tunable via HARNESS_COMMIT_LAG_WARN_S) absorbs the normal seconds between a write and its
# auto-commit. Suppressed under HARNESS_DEMO: the demo/template context deliberately has scratch
# tickets whose session logs outpace the last commit (the demo doesn't run the auto-commit hook, and
# a fresh 60-second-try clone's HEAD is an old upstream commit), so the nudge would be a false alarm
# there. HARNESS_LIVENESS_FORCE re-enables it so the demo's own [R-11 guard] can exercise the real
# check.
check_commit_lag() {
  { [[ -z "${HARNESS_DEMO:-}" ]] || [[ -n "${HARNESS_LIVENESS_FORCE:-}" ]]; } || return 0
  git -C "$WORK_ROOT" rev-parse HEAD >/dev/null 2>&1 || return 0
  ...
  (( newest_session > 0 && commit_epoch > 0 && newest_session > commit_epoch + lag_margin )) \
    || return 0
  msg="recent session activity (newest entry $newest_ts) is newer than the last commit"
  msg="$msg ($(git -C "$WORK_ROOT" log -1 --format=%cr)) —"
  msg="$msg the auto-commit hook may not be firing. Fix: check your hook config"
  msg="$msg (_harness/hooks/hooks.example.json) and that writes are being committed."
  warn "commit-lag" "$msg"
  return 0
}

# check_repo_size — repo-size nudge (issue #16): the record repo grows with every auto-write commit
# and every tracked-notebook revision, so .git creeps up over months of use. Surface it when it gets
...
check_repo_size() {
  ...
  if (( git_kb > git_warn_mb * 1024 )); then
    msg="the record repo's .git is ${git_mib} MiB (working tree ${work_mib} MiB)."
    msg="$msg This grows with every auto-write commit and tracked notebook revision."
    msg="$msg Run _harness/scripts/harness-housekeeping.sh to repack and reclaim space"
    msg="$msg (tune the threshold with HARNESS_GIT_WARN_MB, default 50)."
    warn "git-bloat" "$msg"
    return 0
  fi
  echo "OK: record repo .git ${git_mib} MiB (working tree ${work_mib} MiB) —" \
       "under the ${git_warn_mb} MiB housekeeping threshold."
  return 0
}

# check_hooks — the hooks config parses. HARNESS_HOOKS_FILE overrides the path (flexibility +
# testable).
check_hooks() {
  ...
  # untouched (this branch is dormant on Linux/macOS/WSL and UNWITNESSED here — it needs a Git-Bash
  # host).
...
try: json.load(f)
  ...
  case "$hk_rc" in
    ...
    3)
      echo "FAIL: hooks config could not be READ at '$hooks'" \
           "(path or permission problem, not JSON). Fix: check the path exists and is readable" \
           "(under Git Bash a /c/... path may need cygpath -w)."
      fails=$((fails+1)) ;;
    *)
      echo "FAIL: hooks config is invalid JSON." \
           "Fix: repair '$hooks' (git history has the last good copy)."
      fails=$((fails+1)) ;;
  esac
  return 0
}

# check_agents — _agents/ is the roster; deployed copies must match source.
check_agents() {
  for a in "${CORE[@]}"; do
    [[ -f "$WORK_ROOT/_agents/$a.agent.md" ]] || {
      echo "FAIL: core agent $a.agent.md missing from _agents/." \
           "Fix: git -C '$WORK_ROOT' checkout -- '_agents/$a.agent.md'"
      fails=$((fails+1))
    }
  done
  shopt -s nullglob
...
# sweep_knowledge — GAK staleness sweep (#72) + the undated-note WARN. A note's 'Last reviewed:'
# date is a claim about currency that, until now, nothing read. Status reads it (pure date
# arithmetic) and nags when the note ages past HARNESS_KNOWLEDGE_STALE_DAYS (ruling 5a, default 90 —
# was a hardcoded 183; #72 folds the sweep into status per ruling 5b and tightens it to a quarter).
# It PRESCRIBES the next act BY NAME: the knowledge-curator re-verifies or culls (this repo
# prescribes, it does not merely observe). A note with NO 'Last reviewed:' line at all draws its OWN
# WARN — a date is a claim, and a missing one is a claim that cannot be checked. A note whose line
# is still the placeholder (e.g. a template's 'YYYY-MM-DD') is a skeleton, not a filled note, so the
# sweep stays silent on it.
...
sweep_knowledge() {
  ...
  while IFS= read -r f; do
    rel="${f#$WORK_ROOT/}"
    d=$(grep -m1 -oE 'Last reviewed: [0-9]{4}-[0-9]{2}-[0-9]{2}' "$f" \
        | grep -oE '[0-9-]{10}' || true)
    if [[ -n "$d" ]]; then
      age=$(( (now - $(epoch_from_date "$d")) / 86400 ))
      (( age > HARNESS_KNOWLEDGE_STALE_DAYS )) \
        && echo "WARN: stale knowledge ($age days, reviewed $d): $rel —" \
                "knowledge-curator: re-verify or cull (history keeps it)."
    elif grep -q 'Last reviewed:' "$f"; then
      # a 'Last reviewed:' line but not a real date (placeholder/template skeleton) → stay silent
      :
    else
      echo "WARN: undated knowledge: $rel carries no 'Last reviewed:' date —" \
           "a date is a claim, a missing one cannot be checked." \
           "knowledge-curator: review and stamp it."
    fi
  done < <(find "$WORK_ROOT/General AI-Knowledge" -name '*.md' -type f 2>/dev/null || true)
  return 0
}

# report_tickets — per-ticket summary, matched against the SHARED $TICKET_RE so the summary and the
# validator recognise the exact same set (a name valid under the expanded grammar must appear here,
# not fall through into the WARN sweep below).
...
report_tickets() {
  ...
  while IFS= read -r name; do
    ...
    live=0
    [[ -d "$ak" ]] \
      && live=$(find "$ak" -maxdepth 1 -name '*.md' ! -name '_index.md' 2>/dev/null | wc -l)
    echo "OK: $name — last session ${latest:-none}, knowledge files: $live."
    report_ticket_size "$name"
  done < <(ticket_names)
  return 0
}

# report_ticket_size — the tracked ROOT of one ticket = its total minus the git-ignored bulk (Logs/,
# Dump/). du -sk is portable; du --exclude is GNU-only, so subtract. GUARD each subdir du with
# [[ -d ]] — an absent Logs/ or Dump/ makes du exit non-zero, and (pipefail + set -e) would abort
# the roster loop, the exact class fixed at #37. Absent subdir -> 0.
report_ticket_size() {
  local name="$1"   # the caller's loop variable has the same value; local keeps it that way
  t_total=$(du -sk "$WORK_ROOT/Tickets/$name" 2>/dev/null | awk '{print $1}')
  t_logs=0
  [[ -d "$WORK_ROOT/Tickets/$name/Logs" ]] \
    && t_logs=$(du -sk "$WORK_ROOT/Tickets/$name/Logs" 2>/dev/null | awk '{print $1}')
  t_dump=0
  [[ -d "$WORK_ROOT/Tickets/$name/Dump" ]] \
    && t_dump=$(du -sk "$WORK_ROOT/Tickets/$name/Dump" 2>/dev/null | awk '{print $1}')
  t_root_kb=$(( ${t_total:-0} - ${t_logs:-0} - ${t_dump:-0} ))
  (( t_root_kb > ticket_warn_mb * 1024 )) || return 0
  t_root_mib=$(awk -v k="$t_root_kb" 'BEGIN{ printf "%.1f", k/1024 }')
  msg="Tickets/$name tracks ${t_root_mib} MiB in its root"
  msg="$msg (excluding the ignored Logs/ and Dump/). Large scratch or dropped inputs belong in"
  msg="$msg Dump/ (git-ignored), or add a personal, uncommitted ignore to .git/info/exclude —"
  msg="$msg keep the tracked ticket lean. Tune with HARNESS_TICKET_WARN_MB (default 5)."
  warn "ticket-root:$name" "$msg"
  return 0
}

# sweep_unrecognised — surface, never enforce (Model 1): a folder that HOLDS a ticket record but
# whose name the grammar doesn't recognise is silently skipped by the validator — so a user could
...
sweep_unrecognised() {
  for d in "$WORK_ROOT/Tickets"/*/; do
    ...
    # recognised, no pending marker → validated elsewhere, nothing to surface
    [[ "$name" =~ $TICKET_RE ]] && continue
    # hand-made folder the user opted out of via .not-a-ticket
    ticket_silenced "$d" && continue
    # hand-made ticket-bearing folder → the silenceable WARN; anything else (a scratch dir with
    # no ticket content) stays silent
    if ticket_bearing "$d"; then
      msg="Tickets/$name holds a .md record but doesn't match the recognised ticket pattern,"
      msg="$msg so it isn't validated. If it's a ticket, rename to match or edit the pattern;"
      msg="$msg if not, run: touch 'Tickets/$name/.not-a-ticket' to silence this."
      warn "unrecognised:$name" "$msg"
    fi
  done
  return 0
}

# warn_pending — the two forms of the pending nag, split on whether the folder name conforms yet.
warn_pending() {
  local name="$1"   # the caller's loop variable has the same value; local keeps it that way
  if [[ "$name" =~ $TICKET_RE ]]; then
    ...
    msg="Tickets/$name looks complete (the name conforms) but still carries a .ticket-pending"
    msg="$msg marker. Remove it to finish: rm 'Tickets/$name/.ticket-pending'"
    warn "pending-complete:$name" "$msg"
    return 0
  fi
...
# main — the whole report, in the order it has always printed. The last two steps are #71's: print
# any state-degradation note, then persist the first-seen record (single write, atomic, fails-open).
# That happens AFTER the full report so a bookkeeping failure never truncates it, and it does not
...
main() {
  ...
  warn_state_sync
  (( fails == 0 )) && echo "OK: estate healthy." \
    || { echo "FAIL: $fails issue(s) above — each line names its fix."; exit 1; }
}
