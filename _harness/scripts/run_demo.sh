...
# [#44 hooks-schema] STRUCTURAL check of the verified-schema CANDIDATE config — it must parse and
# carry the deployment-proven shape: top-level "version", the three camelCase events NESTED UNDER
# a "hooks" wrapper (NOT top-level — the proven v4 config and hooks.example.json both wrap them),
# entries keyed on "bash" with NO legacy "command"/"toolFilter". This is STRUCTURE ONLY: it does
# NOT and must not pretend to witness a hook firing — the live fire stays an honest human-witnessed
# box (#44 cond 3). Revert-provable: drop the "hooks" wrapper, a wrapped event key, or the "bash"
# key and this guard reds. (A guard that passed a wrapper-less config is exactly the bug to catch.)
if ! HS44_OUT=$(python3 - "_harness/hooks/harness.candidate.json" <<'PY' 2>&1
import json, sys
...
PY
  ...
  echo "BUG [#44 hooks-schema]: the verified-schema candidate config failed its structural check:"
  printf '%s\n' "$HS44_OUT"
  exit 1
fi
echo "  ok [#44 hooks-schema] — candidate parses; hooks.{sessionStart,postToolUse,sessionEnd} present; verified 'bash' shape, no legacy command/toolFilter"
