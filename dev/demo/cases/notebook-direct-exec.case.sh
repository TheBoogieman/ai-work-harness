...
# the runner; see dev/scripts/run-demo.sh for the contract.
#
...
case_notebook_direct_exec() {
  if ! estate/_harness/scripts/append-notebook-cell.py "$S/Checks/checks_master.ipynb" \
       "check: direct-exec contract (R-07)" "SELECT 1;"; then
    echo "FAIL: append-notebook-cell.py not directly executable — execute bit or shebang missing." \
      "Fix: git update-index --chmod=+x estate/_harness/scripts/append-notebook-cell.py"
    exit 1
  fi
}
