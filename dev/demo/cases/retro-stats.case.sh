#!/usr/bin/env bash
# retro-stats.case.sh — #85 [retro-counts]: retro-stats.sh is a DUMB counter, so a fixture estate
# with KNOWN ticket dates, checks, and promotions must yield KNOWN numbers. SOURCED by the runner.
#
# retro-stats derives the estate it scans from HARNESS_WORK_ROOT, so we point the REAL shipped
...
# subtracted out). Revert-proof: break the counting in retro-stats.sh and this reds on the asserted
...
rs_fixture() {
  local a b c
  echo "--- #85 retrospective: retro-stats counts a fixture estate ---"
  R85=$(mktemp -d)
  git -C "$R85" init -q
  mkdir -p "$R85/Tickets"
  ...
  python3 estate/_harness/scripts/append-notebook-cell.py "$c/Checks/checks_master.ipynb" \
...
rs_counts() {
  ...
  set +e
  R85_OUT=$(HARNESS_WORK_ROOT="$R85" bash estate/_harness/scripts/retro-stats.sh); R85_RC=$?
  set -e
  [ "$R85_RC" -eq 0 ] \
    || { echo "BUG [retro-counts]: retro-stats did not exit 0 on the fixture (rc=$R85_RC):"; \
  ...
  for r85_need in "tickets-closed-total: 3" "  2026-03: 2" "  2026-05: 1" "checks-captured: 2" \
    ...
    printf '%s\n' "$R85_OUT" | grep -qF -- "$r85_need" \
      || { echo "BUG [retro-counts]: retro-stats miscounted the fixture — expected line" \
             "'$r85_need':"; printf '%s\n' "$R85_OUT"; rm -rf "$R85"; exit 1; }
  done
  echo "  ok [retro-counts] — retro-stats counts 3 tickets (2 in Mar, 1 in May), 2 checks," \
