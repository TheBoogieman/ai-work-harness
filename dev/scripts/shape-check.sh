...
#
# Usage:
#   bash dev/scripts/shape-check.sh              # the gate: measure, compare, exit 0 or 1
#   bash dev/scripts/shape-check.sh --regenerate # rewrite the exemption ledger from the
#                                                    # measurement, then exit 0
#   bash dev/scripts/shape-check.sh --report     # print every measured number and exit 0
#
...
SC_MEASURE=dev/scripts/shape-measure.awk
SC_LEDGER=dev/shape-exemptions.txt
...
sc_owner() {
  case "$1" in
    dev/scripts/docs-check.sh) printf '#129\n' ;;
    install.sh) printf '#128\n' ;;
    estate/_harness/scripts/harness-status.sh) printf '#128\n' ;;
    estate/_harness/scripts/check_ticket_log.sh) printf '#128\n' ;;
    *) printf '#175\n' ;;
  esac
}
...
sc_pins() {
  printf '%s\t%s\t%s\n' \
    dev/scripts/docs-check.sh            oh_surface     single \
    estate/_harness/scripts/harness-drill.sh        say            single \
    estate/_harness/scripts/harness-housekeeping.sh dir_kb         single \
    dev/demo/cases/backfill-guards.case.sh  code_has   single \
...
sc_regenerate() {
  {
    printf '# shape-exemptions.txt — GENERATED by dev/scripts/shape-check.sh --regenerate.\n'
